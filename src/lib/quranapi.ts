...
export const toGlobalAyahNumber = (surah: number, verse: number): number => {
  const surahLengths = [
    ...
    44, 28, 28, 20, 56, 40, 31, 50, 22, 31, 13, 54, 13, 18, 10, 30, 9, 11, 21,
    26, 11, 16, 23, 21, 13, 11, 20, 21, 38, 17, 16, 26, 24, 27, 33, 26, 30,
    20, 18, 31, 13, 25, 14, 13, 25, 16, 9, 20, 35, 25, 20, 13,
  ];
  let globalNumber = 0;
...
export const searchVerses = async (query: string, size: number = 10) => {
...
export const getAyatsBySituation = async (
  ...
  try {
    // Split query jadi 2 variasi supaya hasil lebih beragam
    const words = searchQuery.split(" ");
    const mid = Math.ceil(words.length / 2);
    const query2 = words.slice(mid).join(" ");

    // Fetch 2 query sekaligus
    const [result1, result2] = await Promise.all([
      searchVerses(searchQuery, limit),
      searchVerses(query2.length > 2 ? query2 : searchQuery, limit),
    ]);

    const raw1: any[] = result1?.search?.results || [];
    const raw2: any[] = result2?.search?.results || [];

    // Gabung dan deduplikasi by verse_key
    const seen = new Set<string>();
    const merged: any[] = [];
    for (const r of [...raw1, ...raw2]) {
      if (!seen.has(r.verse_key)) {
        seen.add(r.verse_key);
        merged.push(r);
      }
    }

    // Ambil maksimal `limit` hasil
    const results = merged.slice(0, limit);

    // Enrich dengan full verse data (translation + tafsir)
